...
void compton_scatter_phi()
{
    ...
    // WITH SELECTION
    TH1F *hDphi12_sel = new TH1F("hDphi12_sel","#Delta#phi_{12} (80^{#circ} < #theta < 90^{#circ}, No QE);#Delta#phi (deg);Counts",50,-180,180);
    TH1F *hDphi23_sel = new TH1F("hDphi23_sel","#Delta#phi_{23} (80^{#circ} < #theta < 90^{#circ}, No QE);#Delta#phi (deg);Counts",50,-180,180);
    TH1F *hDphi13_sel = new TH1F("hDphi13_sel","#Delta#phi_{13} (80^{#circ} < #theta < 90^{#circ}, No QE);#Delta#phi (deg);Counts",50,-180,180);
    ...
    for(auto &evt:events)
    {
        ...
        if (g0.theta < 100 || g0.theta > 140) continue;
        if (g1.theta < 100 || g1.theta > 140) continue;
        if (g2.theta < 100 || g2.theta > 140) continue;
    ...
    TF1 *fitFcn = new TF1("fitFcn","[0]*sin(2*TMath::DegToRad()*x)+[1]",-180,180);
   ...
   // hPol_m0->Fit(fit_m0,"R");
    ...
    auto getR = [](TF1* f)
    {
        ...
        double R=(B-A)/(B+A);
        double eR = R*sqrt((eA/A)*(eA/A)+(eB/B)*(eB/B));
    ...
    c->SaveAs("dphi_compton_noq_0_180_sin_50.pdf");
    ...
    c_sel->SaveAs("dphi_compton_noq_80_90_sin_50.pdf");
}
